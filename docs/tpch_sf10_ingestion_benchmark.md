...
2. **In-engine COPY** — `SELECT duckdb_query_udf('COPY ... FROM ...')` runs DuckDB's
...
- **COPY is the fast path.** DuckDB's native CSV reader parses the file directly with
  ...
  or in-process via `duckdb_query_udf`.
...
- **The embedded `memory_limit=1.0 GiB` is the main in-engine COPY bottleneck.** Verified,
  ...
  in-engine. `memory_limit` set via `duckdb_query_udf` is instance-level and persists
  across connections.
...
`COPY` via `duckdb_query_udf`** over `LOAD DATA LOCAL INFILE`, and raise the embedded
